...
// gaestebucheintrag
	...
	if (email.length>0)
		...
		if ((re_email.test(email)==false))
			{
			alert ("Bitte g�ltige Emailadresse eingeben.");
...
// powerkids - vorschl�ge
	...
	if (email.length>0)
		...
		if ((re_email.test(email)==false))
			{
			alert ("Bitte �berpr�fe Deine Emailadresse.");
...
// weiterempfehlung
	...
	if ((re_email.test(email)==false))
		{
		alert ("Bitte g�ltige Absender-Emailadresse eingeben.");
	...
	if ((re_email.test(emailempf)==false))
		{
		alert ("Bitte g�ltige Emailadresse des Empfaengers eingeben.");
...
// newsletter-abo/kuendigung
	...
	if (name.length==0)
		...
		if ((re_email.test(email)==false))
			{
			alert ("Bitte �berpr�fen Sie die Schreibweise Ihrer Emailadresse.");
